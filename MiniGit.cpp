...
void MiniGit::checkout(const std::string& ref) {
    if (branches.count(ref)) {
        head = ref;
        std::cout << GREEN << "Switched to branch: " << ref << RESET << "\n";
    } else if (commits.count(ref)) {
        std::cout << YELLOW << "Detached HEAD at: " << ref << RESET << "\n";
        // For simplicity, not updating files in working directory here
    } else {
        std::cout << RED << "Unknown branch or commit: " << ref << RESET << "\n";
    }
    save();
}

void MiniGit::status() const {
    std::cout << CYAN << "On branch: " << head << RESET << "\n";
    //std::cout << "Staged files:\n";
    for (const auto& f : stagedFiles)
        std::cout << "  " << f << "\n";
}

void MiniGit::saveBlob(const std::string& filename, const std::string& hash) {
    std::ifstream in(filename, std::ios::binary);
    if (!in) {
        std::cerr << RED << "Error: Could not open file " << filename << " for reading." << RESET << "\n";
        return;
    }
    std::ofstream out(".minigit/objects/" + hash, std::ios::binary);
    if (!out) {
        std::cerr << RED << "Error: Could not open blob file for writing." << RESET << "\n";
        return;
    }
    out << in.rdbuf();
}
std::string MiniGit::getTimestamp() const {
